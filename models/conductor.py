...
class Conductor(models.Model):
    ...
    ], string="Tipo de Licencia", required=True)
    points_licencia = fields.Integer(string="Puntos de Licencia", required=True)
    fecha_vencimiento_licencia = fields.Date(string="Fecha de Vencimiento de Licencia", required=True)
    
    # Campos computados
    days_to_expiration = fields.Integer(string="Días para Vencimiento", compute="_compute_days_to_expiration", store=True)
    ...
    state = fields.Selection([
        ...
        ('inactivo', 'Inactivo'),
    ], string='Estado', default='activo', track_visibility='onchange')
    ...
    def action_reserva(self):
        for record in self:
            if record.state == 'activo':
                ...
                raise ValidationError("Solo puedes poner en reserva un conductor activo")
